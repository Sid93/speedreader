...
import { listClips, deleteClip, type Clip } from "@speedreader/storage";

/** The extension's Memory bank: everything saved from the article view,
 *  stored in the extension's own IndexedDB. */
    ...
    return (
      <div className="panel">
        ...
        <p className="meta" style={{ marginTop: 8 }}>
          ...
          it opens the article view with that section pre-selected. Save it and it lands here.
        </p>
      </div>
    );
  }
        ...
        return (
          <div key={c.id} className="panel clip-card">
            ...
            <div className="row" style={{ gap: 8, flexWrap: "wrap" }}>
              ...
              <button onClick={async () => { await deleteClip(c.id); setClips((cs) => cs.filter((x) => x.id !== c.id)); }} title="Remove">🗑</button>
            </div>
          </div>
        );
      })}
